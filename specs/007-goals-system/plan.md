...
Implement long-term goal tracking that spans multiple weeks, with three goal types (Weekly Habit, Recurring Task, Target Amount), progress visualization, and task linking for automatic progress updates. Goals can be personal or shared with a partner.

**Technical Approach**: SQLDelight for goal/progress persistence, kotlinx.datetime for week calculations, Koin DI, Jetpack Compose UI with Material Design 3, existing Partner infrastructure for shared goals.
...
**Storage**: SQLDelight (local), offline-first (sync via existing partner infrastructure for shared goals)
...
**Performance Goals**: 60 fps UI, <100ms UI response, <2s goal progress sync
...
- Existing Partner System (Feature 006) for shared goal sync
...
- [x] **Relationship-First Design**: Shared goals strengthen partnership through common objectives; no surveillance mechanics
- [x] **Weekly Rhythm**: Goals are tracked week-by-week; Weekly Habit resets each week; progress history is weekly
- [x] **Autonomous Partnership**: Each partner owns their personal goals; shared goals are collaborative, not assigned
...
2. Does it respect partner autonomy? ✓ Personal goals are private; shared goals require mutual creation
...
- [x] NO tracking of partner's incomplete tasks - Shared goals show mutual progress, not blame
- [x] NO notifications for partner's task completions (default off) - Not applicable to goals
- [x] NO assigning tasks without acceptance workflow - Goals link to tasks voluntarily
...
| Method/API | Purpose | Preconditions |
|------------|---------|---------------|
| `partnerRepository.getPartner()` | Check partner for shared goals | Partnership exists |
| `partnerRepository.observePartner()` | React to partner changes | User authenticated |

### New Repositories
| Repository | Purpose | Key Methods |
|------------|---------|-------------|
| `GoalRepository` | Goal lifecycle | `createGoal()`, `observeGoals()`, `updateGoal()`, `deleteGoal()`, `recordProgress()` |
