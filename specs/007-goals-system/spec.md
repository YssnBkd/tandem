...
### User Story 1 - View Personal and Partner Goals (Priority: P1)

As a user, I can view my personal goals and my partner's goals in a dedicated Goals tab, organized by ownership.
...
2. **Given** my partner has goals, **When** I select "Partner's" segment, **Then** I see my partner's goals in read-only view
3. **Given** I have no goals, **When** I navigate to Goals tab, **Then** I see an empty state with guidance to create my first goal
4. **Given** I view my partner's goals, **When** I tap on a partner's goal card, **Then** I see the goal detail in read-only mode (no edit/delete options)

---

### User Story 2 - Create a New Goal (Priority: P1)

As a user, I can create a new goal specifying its name, icon, type (Weekly Habit, Recurring Task, or Target Amount), and duration.
...
3. **Given** I am creating a Target Amount goal, **When** I set a total target number, **Then** the goal tracks cumulative progress toward that number
4. **Given** I have a partner, **When** I create a goal, **Then** my partner can see my goal in their "Partner's" segment
...
As a user, I can link my tasks to my own goals so that completing tasks automatically updates goal progress.
...
1. **Given** I am editing a task, **When** I tap the goal picker, **Then** I see a list of my active goals to choose from (not partner's goals)
...
1. **Given** I tap on my goal card, **When** the detail screen opens, **Then** I see full goal info including name, icon, type, duration, and targets
...
4. **Given** I want to modify my goal, **When** I tap Edit on the detail screen, **Then** I can update the goal's properties
5. **Given** I tap on my partner's goal, **When** the detail screen opens, **Then** I see goal info and history but no Edit/Delete options

---

### User Story 6 - Edit and Delete Goals (Priority: P3)

As a user, I can edit my goal properties or delete my goals that are no longer relevant.
...
1. **Given** I am on my goal detail screen, **When** I tap Edit and modify the goal name, **Then** the updated name is saved and reflected everywhere
2. **Given** I want to remove my goal, **When** I tap Delete and confirm, **Then** the goal is removed and linked tasks lose their goal reference
3. **Given** I edit my goal, **When** I save changes, **Then** my partner sees the updated goal in their "Partner's" view
...
- What happens when a goal's duration expires?
  ...
  - Completed/Expired goals remain visible in the "Yours" segment with a status indicator
  - Users can filter to show only active goals (default) or all goals including completed/expired
  - No separate archive view in MVP; completed goals simply show their final status
...
- What happens when a user unlinks from their partner?
...
- What happens when viewing partner's goals while offline?
  - Last synced partner goals are displayed from local cache (PartnerGoal table)
  - A "Last updated: [timestamp]" indicator shows when data was last synced
  - Cache is considered stale after 1 hour; stale indicator changes color/style
  - Goals sync automatically when connectivity is restored (no manual refresh required)
  - Partner goals are cached indefinitely (no expiration) to ensure offline access
...
- **FR-004**: System MUST allow users to edit their own goal properties after creation
- **FR-005**: System MUST allow users to delete their own goals with confirmation
- **FR-006**: System MUST limit users to a maximum of 10 active goals (completed/expired goals do not count toward this limit)

**Goal Display**
- **FR-007**: System MUST display goals in a dedicated Goals tab
- **FR-008**: System MUST segment goals into "Yours" (own goals) and "Partner's" (partner's goals, read-only) views
...
- **FR-011a**: System MUST visually distinguish completed/expired goals from active goals in the goal list
- **FR-011b**: System SHOULD provide a filter to toggle visibility of completed/expired goals (default: show all)
- **FR-012**: System MUST display partner's goals in read-only mode (no edit/delete actions)

**Progress Tracking**
- **FR-013**: System MUST calculate Weekly Habit progress as completions / target per week
- **FR-014**: System MUST calculate Recurring Task progress as completed/not completed for current week
- **FR-015**: System MUST calculate Target Amount progress as cumulative completions / total target
- **FR-016**: System MUST reset weekly progress for Weekly Habit goals at the start of each week
- **FR-017**: System MUST preserve week-by-week progress history for goals

**Task-Goal Linking**
- **FR-018**: System MUST allow optional goal reference on tasks (only to user's own goals)
- **FR-019**: System MUST display a goal badge on tasks linked to goals
- **FR-020**: System MUST update goal progress when linked tasks are completed
- **FR-021**: System MUST allow users to select from their own active goals when linking a task

**Goal Detail View**
- **FR-022**: System MUST display full goal information on a detail screen
- **FR-023**: System MUST display week-by-week progress history on detail screen
- **FR-024**: System MUST display list of linked tasks on detail screen (only for own goals)
- **FR-025**: System MUST provide edit and delete options on detail screen for own goals only

**Partner Goal Visibility**
- **FR-026**: System MUST sync partner's goals for read-only visibility via existing partnership infrastructure
- **FR-027**: System MUST allow users to view their partner's goals and progress
- **FR-028**: System MUST NOT allow users to edit, delete, or link tasks to partner's goals

**Week Planning Integration**
- **FR-029**: System MUST provide goal-based task suggestions during week planning (based on own goals only)
- **FR-030**: System MUST display a "Based on your goals" section in planning view

### Key Entities

- **Goal**: Represents a user's long-term objective. Contains identity (id, name, icon), configuration (type, targetPerWeek, targetTotal, durationWeeks), state (currentProgress, startWeekId), ownership (ownerId), and metadata (createdAt). Related to Tasks via optional linking and to Users via ownership. Each goal belongs to exactly one user.
...
- **Task (extended)**: Existing Task entity gains optional linkedGoalId field to associate tasks with goals (only the user's own goals).
...
- **SC-003**: Users can navigate between personal and partner goal views in under 1 second
...
- **SC-006**: Partner's goals are visible within 5 seconds of partnership being established or connectivity restoration
- **SC-006a**: Partner goals remain accessible offline with cached data and "Last updated" indicator
...
- Q: Can users edit or contribute to their partner's goals? → A: No, partner goals are read-only. Each user owns and manages only their own goals.
...
- Partner system exists for partner goal visibility (Feature 006)
